def order_updates_given_rules(update: list, rules: dict) -> list:
    """
    Order the updates correctly by taking into account the given rules

    Params:
        - update: list with the numbers to update
        - rules: rules to be followed in order to re-order the update

    Returns:
        The page update correctly ordered
    """
    for it in range(len(update)):
        try:
            rule = rules[update[it]]
            for item in range(len(update)):
                if update[it] != update[item] and update[item] in rule and it > item:
                    # Change the order
                    aux = update[it]
                    update[it]=update[item]
                    update[item]=aux
                else:
                    continue
    ...
    with open("input.txt", "r") as f:
        ...
        for update in page_numbers_to_update:
            ...
            if correct_order:
                ...
                update = order_updates_given_rules(update=update, rules=ordering_rules_by_number)
                middle_numbers_addition += update[int(len(update)/2)]
